    ...
    try {
		...
		if(isset($postdata) && !empty($postdata))
		{
		  ...
		  if((int)$request->id < 1 ||  trim($request->name) === '' 
			|| trim($request->image) === '' || (int)$request->difficulty < 1 || (int)$request->difficulty > 5)
		  {
			echo 'Incorrect data!';
		  ...
		  // Store.
		  $sql = "UPDATE `recipe` SET `name`='$name', `image`='$image', `difficulty`='$difficulty', `category_id`=$category_id, `country_id`=$country_id WHERE `id_recipe`='{$id}' LIMIT 1";
